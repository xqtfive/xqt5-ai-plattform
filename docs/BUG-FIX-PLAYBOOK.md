...
|---|---|---|
| #87 | `_call_anthropic` `block["text"]` KeyError bei non-text Block | **REFUTED 2026-05-18** — Code an `llm.py:282` filtert bereits `block.get("type") == "text"` vor dem Index-Zugriff; KeyError-Pfad nicht erreichbar. Das damit assoziierte „silent-empty"-Problem ist als #125 separat dokumentiert + gefixt |
...
- **#143 (2026-05-18)** — Image-Gen `parameters`-Bypass — zwei-Schicht-Defense: (a) `models.py` neuer `ImageGenerationParameters(BaseModel)` mit `model_config=ConfigDict(extra='forbid')` und KEINEN Feldern in v1 (Frontend sendet heute keine Parameter — strict-Schema bricht nichts), `ImageGenerationRequest` selbst auch `extra='forbid'`. (b) `image_gen.py` neue per-Provider-Allowlist-Konstanten `_OPENAI_IMAGE_ALLOWED` und `_XAI_IMAGE_ALLOWED` (beide in v1 leer) + Helper `_filter_provider_parameters(provider, params)` der nicht-allowlisted Keys droppt und warnt; aufgerufen direkt nach Provider-Resolve in `generate_image_for_user`, damit sowohl `create_pending_image` als auch `_call_provider` den gefilterten Dict bekommen (Audit-Honesty). `main.py:2346` Route-Handler ruft `body.parameters.model_dump()` für den dict-Übergang. Schließt Cost-Bypass-Vektor (User konnte `parameters={"model":"gpt-image-1","n":10,"size":"1536x1024"}` posten → Provider rechnet 10×teuer, aber Daily-Cap nur 1×billig). Phase-A 3-Agent + neue Phase-A-Regeln. Details: `IMPLEMENTIERT.md` „Bugfix #143 (2026-05-18)".
- **#57/#255 + #82/#235/#201 + #111 + #125 + #87-Refutation (G3 Cluster, 2026-05-18)** — Vier zusammenhängende Provider-Hygiene-Fixes in einem Commit, alle in `backend/app/llm.py` + Sibling-Sites: (a) zentraler `_safe_provider_error(provider, status, raw_body)`-Helper der nur strukturiertes `error.message` aus dem JSON-Envelope extrahiert (Anthropic/Google/OpenAI/Mistral-Form), fällt zurück auf generic `HTTP {status}`, loggt volles Body via `logger.warning` server-side — angewendet an allen 8 LLMError-Sites in llm.py plus Sibling-Sites `rag.py:608`, `providers.py:190`, `main.py:1543`; (b) Google API-Key umgezogen von `?key=` URL-Query nach `x-goog-api-key`-Header an 4 Sites (`llm.py:296, 532`, `providers.py:174`, `main.py:1536`); (c) `_check_google_block(data)` raised explizit `LLMError` bei `finishReason ∈ {SAFETY, RECITATION, BLOCKLIST, PROHIBITED_CONTENT, SPII}` oder `promptFeedback.blockReason` (in Stream-Variante via `final_finish_reason`-Tracking); (d) Anthropic non-text-Block-Detection in `_call_anthropic` (raises bei leerem text mit Block-Types) + `_stream_anthropic` (`text_yielded`-Tracking + `seen_block_types` + `stop_reason` capture, raises wenn nichts geliefert); (e) #87 als REFUTED dokumentiert (Type-Filter an L282 deckt KeyError-Pfad bereits ab). **P0 für PROD-Catchup:** `GOOGLE_API_KEY` rotieren nach Deploy (Red-Team-Verdict: Key potenziell schon via Coolify-Logs/Tracebacks exponiert). Phase-A mit 4 Opus-Agenten nach neuen Dispatch-Regeln (incl. Framework-Runtime-Researcher mit Starlette+uvicorn-Source-Trace, Red-Team-Refuter der 3 zusätzliche Sibling-Sites identifizierte). Details: `IMPLEMENTIERT.md` „Bugfix G3 Cluster (2026-05-18)".
...
| #57 / #255 | Provider-Error-Body wortwörtlich in `LLMError` → bubble an Client | `llm.py:251, 278, 301, 382, 411, 455, 498, 539` + `rag.py:608` + `providers.py:190` + `main.py:1543` (siblings co-fixed) | FIXED 2026-05-18 (zentraler `_safe_provider_error`-Helper) | llm.py alle 8 Call-Sites, main.py Error-Path |
...
| #82 / #235 / #201 | Google API-Key in URL-Query (kein Header) + Leak via Error-Body | `llm.py:296, 532` + `providers.py:174` + `main.py:1536` (4 Sites total) | FIXED 2026-05-18 (alle 4 auf `x-goog-api-key`-Header) — **P0 Red-Team-Verdict, Key-Rotation post-Deploy obligatorisch** | llm.py 2 Stellen — `x-goog-api-key`-Header umstellen |
| #87 | `_call_anthropic` KeyError bei missing `text`-Field | `llm.py:282` | REFUTED 2026-05-18 (siehe §4 Refutations-Register) | KeyError-Pfad nicht erreichbar wegen bestehendem type-Filter |
| #111 | Google SAFETY-Block silent empty Content | `llm.py:307, 554–560` | FIXED 2026-05-18 (`_check_google_block` + Stream-final_finish_reason-Tracking) | llm.py — finishReason/blockReason-Check + Error-Raise |
| #125 | Anthropic non-text-Block (thinking, tool_use) → empty content persisted | `llm.py:_call_anthropic`/`_stream_anthropic` | FIXED 2026-05-18 (text_yielded-Tracking + Block-Type-Surface in LLMError) | llm.py — Block-Type-Handling |
...
| #143 | Image-Gen `parameters`-Bypass (`payload.update(parameters)`) | `image_gen.py:154–155, 207–208` + `models.py:221` (audit-Zeitstand, post-fix `image_gen.py:180, 233`) | FIXED 2026-05-18 (zwei-Schicht-Defense: Pydantic `extra='forbid'` + runtime per-Provider-Allowlist) | image_gen.py + models.py Allowlist |
