...
9. **User-Input-Dicts brauchen typisierte Pydantic-Modelle mit `extra='forbid'`, nicht `Dict[str, Any]`.** Jedes Request-Body-Feld das ein „Sack an Optionen" akzeptiert (z.B. provider-Parameter, Suchfilter, RAG-Settings-Overrides) muss als separate `BaseModel`-Klasse mit Literal-typisierten Feldern und `model_config = ConfigDict(extra='forbid')` deklariert sein. Zusätzlich: das umschließende Request-Model sollte selbst auch `extra='forbid'` haben. Pure `Dict[str, Any]` ist ein versteckter Vektor für Server-Field-Overwrites (z.B. `payload.update(parameters)` lässt User Server-Defaults clobbern). Wenn der Dict-Inhalt dann an externe APIs weitergegeben wird, muss zusätzlich eine Runtime-Allowlist als defense-in-depth angewendet werden (siehe `image_gen._filter_provider_parameters` als Referenz-Implementation). Hintergrund: Bug #143 (2026-05-18) — Cost-Bypass über `parameters={"model":"...","n":10}`. Geschwister-Findings unter „P7 Pydantic-permissive" im Audit: #158, #159, #165, #182 — alle teilen das Pattern.
10. **Provider-Error-Bodies dürfen nie wortwörtlich in User-sichtbare Fehlermeldungen einfließen.** Jeder HTTP-Call zu einem externen Provider (LLM-API, Embedding-API, OCR-API, etc.) der einen Fehlerpfad hat muss den Body sanitizen, bevor er in eine Exception, einen SSE-Frame, oder eine `HTTPException(detail=...)` einfließt. Referenz-Implementation: `llm._safe_provider_error(provider, status, raw_body)` — extrahiert nur `error.message` aus dem JSON-Envelope, fällt zurück auf generic `HTTP {status}`, loggt den vollen Body via `logger.warning` server-side (NIE in `app_audit_logs`). Hintergrund: Bugs #57/#255 (2026-05-18) — Provider-Bodies enthielten echo'd Request-URLs (Google `?key=`), Prompt-Fragmente, und interne Request-IDs, die alle via SSE-Error-Frame oder JSON-detail zum Browser gelangten. Faustregel: wenn du an irgendeiner Stelle `resp.text[:N]` in einen User-facing String interpolierst, machst du dasselbe Anti-Pattern.
