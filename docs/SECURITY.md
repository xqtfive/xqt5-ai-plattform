...
**Provider-Error-Body-Sanitization (2026-05-18, Fixes #57/#255, #82/#235/#201, #111, #125).** Provider-API-Fehler (Anthropic, Google, OpenAI, Mistral, Azure, xAI) durchlaufen einen zentralen Helper `llm._safe_provider_error(provider, status, raw_body)`, der nur das strukturierte `error.message`-Feld aus dem JSON-Envelope extrahiert; alles andere wird verworfen. Der vollständige Body wird via `logger.warning` server-side geloggt (Coolify-Stdout, niemals `app_audit_logs`). Vor diesem Fix war `resp.text[:500]` direkt in `LLMError` interpoliert und über SSE bzw. HTTPException-Detail zum Browser durchgereicht — User-sichtbarer Provider-Body, inklusive eventuell echo'd Request-URLs mit eingebetteten API-Keys (siehe Google-Fall unten). Newlines werden entfernt (SSE-Safety), Length cap 200 Chars.

**Google API-Key in Header statt URL (2026-05-18, Fix #82/#235/#201).** Vor diesem Fix wurde der Google-API-Key als `?key={api_key}`-Query-Parameter an `:generateContent` und `:streamGenerateContent` mitgegeben. Konsequenzen: Key in `httpx.RequestError.__str__`, in Coolify-Access-Logs, in `_provider_body`-Echos, und potenziell in jedem 4xx-Fehlerpfad sichtbar. Phase-A-Red-Team klassifizierte das als P0 — Key war potenziell bereits via Container-Logs exponiert. Fix: alle 4 Sites (`llm.py:296`, `llm.py:532`, `providers.py:174`, `main.py:1536`) umgestellt auf `x-goog-api-key`-Header, was die kanonische Auth-Form per Google-AI-Docs ist. **Action-Item:** Der bestehende Google-API-Key muss nach Deploy rotiert werden — siehe `PROD-UPGRADE-PLAYBOOK.md`.

**Anthropic/Google Silent-Empty-Content-Detection (2026-05-18, Fixes #111, #125).** Anthropic `_call_anthropic` und `_stream_anthropic` raised jetzt `LLMError` wenn die Response nur non-text-Blöcke (`thinking`, `tool_use`, `redacted_thinking`, etc.) liefert; Block-Types und `stop_reason` sind im Error-Message für Forensik enthalten. Google `_call_google` und `_stream_google` raised `LLMError` bei `finishReason ∈ {SAFETY, RECITATION, BLOCKLIST, PROHIBITED_CONTENT, SPII}` oder `promptFeedback.blockReason`. Vor diesem Fix produzierten beide Pfade silent-empty-Responses → User sah leere Assistant-Message in DB, bezahlte aber Prompt-Tokens.
